...
package com.liangchengj.amuse.lang.process;

import com.liangchengj.amuse.lang.JDKToolFinder;
import com.liangchengj.amuse.lang.util.JDKTestLib;
...
public final class ProcessTool {
   ...
   * @throws IOException
   */
  public static Process startProcess(String name, ProcessBuilder processBuilder)
      throws IOException {
  ...
  public static Process startProcess(
      String name, ProcessBuilder processBuilder, Consumer<String> consumer) throws IOException {
    try {
   ...
   *
   * <p>It is possible to wait for the process to get to a warmed-up state via {@linkplain
   * Predicate} condition on the STDOUT and monitor the in-streams via the provided {@linkplain
   * Consumer}
   *
   * @param name The process name
   * @param processBuilder The process builder
  ...
  public static Process startProcess(
      ...
      throws IOException, InterruptedException, TimeoutException {
    System.out.println(
        "[" + name + "]:" + processBuilder.command().stream().collect(Collectors.joining(" ")));
    ...
    try {
      if (timeout > -1) {
        if (timeout == 0) {
          ...
          if (!latch.await(JDKTestLib.adjustTimeout(timeout), unit)) {
   ...
   *
   * <p>It is possible to wait for the process to get to a warmed-up state via {@linkplain
   * Predicate} condition on the STDOUT. The warm-up will wait indefinitely.
   *
   * @param name The process name
   * @param processBuilder The process builder
   * @param linePredicate The {@linkplain Predicate} to use on the STDOUT Used to determine the
  ...
  @SuppressWarnings("overloads")
  public static Process startProcess(
      String name, ProcessBuilder processBuilder, final Predicate<String> linePredicate)
      throws IOException, InterruptedException, TimeoutException {
  ...
  public static ProcessBuilder createJavaProcessBuilder(
      boolean addTestVmAndJavaOptions, String... command) {
    ...
    if (addTestVmAndJavaOptions) {
      Collections.addAll(args, JDKTestLib.getTestJavaOpts());
    }
  ...
  /**
   * Executes a test jvm process, waits for it to finish and returns the process output. The default
   * jvm options from jtreg, test.vm.opts and test.java.opts, are added. The java from the test.jdk
   * is used to execute the command.
   *
   * <p>The command line will be like: {test.jdk}/bin/java {test.vm.opts} {test.java.opts} cmds
   *
  ...
  public static OutputAnalyzer executeTestJvm(String... cmds) throws Exception {
    ProcessBuilder pb = createJavaProcessBuilder(JDKTestLib.addTestJavaOpts(cmds));
  ...
  /**
   * Executes a process, waits for it to finish and returns the process output. The process will
   * have exited before this method returns.
   *
  ...
  public static OutputAnalyzer executeCommand(String... cmds) throws Throwable {
    ...
    OutputAnalyzer analyzer = ProcessTool.executeProcess(cmds);
  ...
  public static OutputAnalyzer executeCommand(ProcessBuilder pb) throws Throwable {
    String cmdLine =
        pb.command().stream()
            .map(x -> (x.contains(" ") || x.contains("$")) ? ("'" + x + "'") : x)
            .collect(Collectors.joining(" "));
    System.out.println("Command line: [" + cmdLine + "]");
    OutputAnalyzer analyzer = ProcessTool.executeProcess(pb);
    System.out.println(analyzer.getOutput());
    return analyzer;
  }
